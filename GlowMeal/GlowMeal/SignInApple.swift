...
class SignInApple: NSObject{
    ...
    private func SignInWithAppleFlow(completion: @escaping (Result<SignInAppleResult, Error>) -> Void) {
        UIApplication.getTopViewController { topVC in
...
extension UIApplication {
    class func getTopViewController(completion: @escaping (UIViewController?) -> Void) {
        DispatchQueue.main.async {
            let base = getKeyWindow()?.rootViewController
            completion(getTopViewController(base: base))
        }
    }
    ...
    private class func getKeyWindow() -> UIWindow? {
        ...
        if Thread.isMainThread {
            keyWindow = getKeyWindowInternal()
        } else {
    ...
    private class func getKeyWindowInternal() -> UIWindow? {
        if #available(iOS 13, *) {
